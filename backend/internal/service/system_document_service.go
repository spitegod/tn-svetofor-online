...
func (s *SystemDocumentService) Export(ctx context.Context, filter model.SystemDocumentFilter) ([]byte, error) {
	...
	headers := []string{"Шифр", "Название системы", "Класс", "Куратор", "Комментарий"}
	for index, header := range headers {
		_ = file.SetCellValue(sheet, fmt.Sprintf("%c1", 'A'+index), header)
	}
	for index, row := range rows {
		excelRow := index + 2
		values := []string{row.Code, row.SystemName, row.SystemClass, row.Curator, row.Comment}
	...
	_ = file.SetColWidth(sheet, "C", "E", 24)
